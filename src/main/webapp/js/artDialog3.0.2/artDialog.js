...
$.getScript = function(src, fn, doc) {
	...
	script.src = src;
	// 不插入head是防止script标签在DOM顺序不对，会影响相关操作
...
// @param {String} HTML片段
// @return {undefined}
/*$.txt2script = function(content){
	if (typeof content !== 'string') return;
	var script = /(?:<script([^>]*)?>)((\n|\r|.)*?)(?:<\/script>)/ig,
		src = /\ssrc=([\'\"])(.*?)\1/i,
		//type = /\stype=([\'\"])(.*?)\1/i,
		match;
	while (match = script.exec(content)) {
		var attrs = match[1],
		srcMatch = attrs ? attrs.match(src) : false;
		if (srcMatch && srcMatch[2]) {
			$.getScript(srcMatch[2]);
		} else
		if (match[2] && match[2].length > 0) {
			window.execScript ? window.execScript(match[2]) : window.eval(match[2]);
		};
	};
};*/
...
(function(){
	...
	path = path[path.length-1].src.replace(/\\/g, '/');
	
	// 当前外链js所在路径
	$.getPath = path.lastIndexOf('/') < 0 ? '.' : path.substring(0, path.lastIndexOf('/'));
	
	// 当前外链js地址
	$.getUrl = path.split('?')[0];
	
	// 当前外链js参数
	$.getArgs = path.split('?')[1] || '';
})();
...
(function(){
	...
	// 元素拖动
	// 参数options对象成员：
	// fixed: 是否静止定位, limit: true 是否限制挪动范围,
	// downFn: 鼠标按下的回调函数, upFn: 鼠标松开的回调函数,
	// zIndex: 1 初始叠加高度, showTemp 超过此面积的层使用替身代替拖动
...
// 支持IE6与现代浏览器的fixed与absolute切换
// fixed状态的元素如果要拖，IE6要如下处理：准备拖动的时候需要设置为absolute，结束拖动再设置回fixed
var position;
$(function(){
...
var count = 0,
	...
	$iframe_mask, $temp_wrap, $temp, $overlay, topBoxApi, lockBoxApi,
	...
	var lock = {
		on: function(){
			...
			ui.dialog.bind('keydown', function(event){
				var key = event.keyCode;
				(key === 9 || key === 38 || key === 40) && $.stopBubble(event);
			});
	...
	if ($.isIE6) {
		...
		$.each(list, function(i, elem){
			...
			var png = $(elem).css('ie6png'),
				pngPath = $.dialog.defaults.path + '/skin/' + data.skin + '/' + png;
			if (png) {
...
var allReady = function(){
	...
	// 载入核心CSS文件
	if (!dialogReady) return $.getStyle($.dialog.defaults.path + '/core/layout.css', function(){
		dialogReady = true;
		allReady();
	});
...
$.fn.dialog.confirm = function(content, yes, no){
	return $.dialog({
			...
			content: content,
			yesFn: function(that){
				return yes.call(this, that);
			},
			noFn: function(that){
				return no && no.call(this, that);
			}
		});
};
// 提问
...
$.fn.dialog.prompt = function(content, yes, value){
	...
	return $.dialog({
		...
		focus: input,
		yesFn: function(that){
			return yes && yes.call(this, that.art('#' + input)[0].value, that);
		},
		noFn: true
	});
};
		...
		data = {
			...
			initFn: function(that){
				...
				load = function(){
					...
					//TODO
					/*
					$.dialog.inner($iframe[0].contentWindow, function(){
						// 给当前对话框iframe里面扩展一个关闭方法
						$iframe[0].contentWindow.art.fn.dialog.close = function(){
							api.close();
						};
					});*/
					...
					if (api.data.width === 'auto' && api.data.height === 'auto') try{
						var doc = $.doc($iframe[0].contentWindow);
					...
					api.loading.off();
					opt.initFn && opt.initFn.call(api, that);
				};
				
				$iframe.bind('load', load);
			},
			closeFn: function(that){
				$iframe.unbind('load', load);
		
				// 重要！需要重置iframe地址，否则下次出现的对话框在IE6、7无法聚焦input
				// IE注销iframe仍然留在内存中，置换src是最容易解决的方法
				$iframe[0].src = 'javascript:false';
				opt.closeFn && opt.closeFn.call(api, that);
			}
		};
	
	// 回调函数第三个参数指向iframe内部window对象
	if (opt.yesFn) data.yesFn = function(that){
		return opt.yesFn.call(this, $iframe[0].contentWindow, that);
	};
	if (opt.noFn) data.noFn = function(that){
		return opt.noFn.call(this, $iframe[0].contentWindow, that);
	};
